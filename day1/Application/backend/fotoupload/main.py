...
async def images(image_name):
    try:
        ...
        raise HTTPException(status_code=404, detail="Your picture was not found, just try to upload again 🤓")
    return StreamingResponse(blob.chunks())


@app.post("/upload/")
async def upload(file: UploadFile = File(...)):
    container_client.upload_blob(file.filename, file.file, blob_type="BlockBlob", overwrite=True)
